...
// 			getLastAccessedTime() : 클라이언트에 마지막으로 요청 보낸 시간을 long형으로 반환(msec)
			long last_time = session.getLastAccessedTime();
// 			getCreationTime() : 세션이 만들어진 시간(msec)
			long create_time = session.getCreationTime();
// 			웹사이트에 머무른 시간(분)
			long time_used = (last_time - create_time) / 60000; // 60 * 1000
// 			getMaxInactiveInterval() : 세션 유지 시간(기본은 30(분) => 초단위)
			int inactive = session.getMaxInactiveInterval() / 60;	// 분으로 계산
// 			처음 생성된 세션이면 참
			boolean b_new = session.isNew();
		%>
		[1] 세션 ID 는 [<%= str_id %>] 입니다.<br><hr>
		[2] 당신이 웹 사이트에 머무른 시간은 [<%= time_used %>] 분입니다.<br><hr>
